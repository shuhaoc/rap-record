...
/*
extern "C" JNIEXPORT jstring JNICALL Java_net_surina_soundtouch_SoundTouch_getVersionString
  (JNIEnv* env, jobject) {
    const char* paramValues[4] = { "me", "/sdcard/_rec/record.wav", "/sdcard/_rec/out.wav", "-t=40" };
    main(4, paramValues);
 ...
 */
extern "C" JNIEXPORT void JNICALL Java_net_surina_soundtouch_SoundTouch_stretch
  (JNIEnv* env, jclass, jstring inputWav, jstring outputWav, jint tempoDelta,
    jint pitchDelta, jint rateDelta, jboolean speech) {
    WavInFile *inFile;
    WavOutFile *outFile;
    RunParameters *params;
    SoundTouch soundTouch;

    LOGV("%s", SoundTouch::getVersionString());

    // Parse command line parameters
    jboolean copy;
    params = new RunParameters();
    params->inFileName = const_cast<char*>(env->GetStringUTFChars(inputWav, &copy));
    params->outFileName = const_cast<char*>(env->GetStringUTFChars(outputWav, &copy));
    params->tempoDelta = tempoDelta;
    params->pitchDelta = pitchDelta;
    params->rateDelta = rateDelta;
    params->speech = speech ? TRUE : FALSE;
    try 
    {
        // Open input & output files
        openFiles(&inFile, &outFile, params);
    ...
    env->ReleaseStringUTFChars(inputWav, params->inFileName);
    env->ReleaseStringUTFChars(outputWav, params->outFileName);
    delete params;
}
